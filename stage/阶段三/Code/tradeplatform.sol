...
contract tradeplatform{
    ...
    function buyitnowprice(uint id) payable public returns(string, bool){
    ...
    function sellerconfirm (uint id) public returns(bool) {
    ...
    function extraction (uint id) public view returns(string, bool){
        ...
        if(msg.sender == shelves[id].HighestBidder && (tradestate == state.locked||tradestate == state.completed))
        {
    ...
    function takemoney () public{
        require(tradestate == state.completed && msg.sender == seller,"无权取钱");
    ...
    function querygoodbyid(uint id) public view returns(good){
        require(id<goodcount,"错误的货品编号");
        good memory thegood;
        thegood = shelves[id];
        thegood.key = "******";
